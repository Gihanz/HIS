	...
	$('document').ready(function () {
        ...
        //<!--Edit department-->
         $('.btn_edit').click(function () {
            ...
            var lab = $(this).closest("tr").find('td:eq(2)').text();
            var specimen_type = $(this).closest("tr").find('td:eq(3)').text();
            var test_name = $(this).closest("tr").find('td:eq(4)').text();
            var test_short_name = $(this).closest("tr").find('td:eq(5)').text();
            var is_binary_type=$(this).closest("tr").find('td:eq(6)').text();
			var defult_test_comment=$(this).closest("tr").find('td:eq(7)').text();
			var test_unit=$(this).closest("tr").find('td:eq(8)').text();

            $('#lab_test_id_edit').val(row_id);
            $('#department_name_edit').val(department_name);
            $('#lab_edit').val(lab);
            $('#specimen_type_edit').val(specimen_type);
            $('#test_name_edit').val(test_name);
            $('#test_short_name_edit').val(test_short_name);
            $('#is_binary_type_edit').val(is_binary_type);
			$('#defult_test_comment_edit').val(defult_test_comment);
			$('#test_unit_edit').val(test_unit);
        });

        
        //  <!--Model for update department -->
                 
                $('#btn_update_laboratory_test').click(function () {
                     //alert("TEST!");
                var json = [];
                var obj = {};

                var lab_test_id = $('#lab_test_id_edit').val();
                var department_name = $('#department_name_edit').val();
                var lab = $('#lab_edit').val();
                var specimen_type = $('#specimen_type_edit').val();
                var test_name = $('#test_name_edit').val();
                var test_short_name = $('#test_short_name_edit').val();
				if($('#is_binary_type_edit').val()=="Yes"){
				var is_binary_type=1}else{
				var is_binary_type=0}                
				var defult_test_comment=$('#defult_test_comment_edit').val();
				var test_unit=$('#test_unit_edit').val();
                
                obj ['labTestId'] = lab_test_id;
                obj ['departmentName'] = department_name;
                obj ['lab'] = lab;
                obj ['specimenType'] = specimen_type;
                obj ['testName'] = test_name;
                obj ['testShortName'] = test_short_name;
				obj ['isBinaryType']=is_binary_type;
                obj ['defultTestComment']=defult_test_comment;
				obj ['testUnit']=test_unit;
				
               
                json.push(obj)
                var updatedLaboratoryTestJSONObject = {obj};
                //alert(JSON.stringify(newEmployeeJSONObject));

                $.ajax({
                    type: "POST",
                    url: 'mri_test_fields/UpdateLaboratoryTest',
                    data: {'labTestId': updatedLaboratoryTestJSONObject}, success: function (output) {
