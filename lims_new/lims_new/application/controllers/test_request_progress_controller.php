...
class test_request_progress_controller extends CI_Controller {
    ...
    //     print_r($data['specimen_maxID']);
        ...
        //Chief MLT,Doctor, MLT,Admin ,Doctor 
        if($roleId == "3"||$roleId == "1"||$roleId == "2"||$roleId == "4"||$roleId == "6"||$roleId == "7")
        {
     ...
     //if you want to write it to disk and/or send it as an attachment    
      ...
      public function UpdateTestRequest() {
        if (isset($_POST['testrequest'])) {
            $Data = $_POST['testrequest'];

            $data_string = json_encode($Data);

            $this->load->model('laboratory_test_request_model');
            $ss = $this->laboratory_test_request_model->UpdateTestRequest($data_string);
           ...
           //UpdateTestRequestComment
      public function UpdateTestRequestComment() {
        if (isset($_POST['testrequest'])) {
            $Data = $_POST['testrequest'];
            ...
            $this->load->model('laboratory_test_request_model');
            $ss = $this->laboratory_test_request_model->UpdateTestRequestComment($data_string);
