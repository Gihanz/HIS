...
class Mri_binary_results extends CI_Controller {
    ...
    public function addSingleResult() {
       ...
       public function updateSingleResult1() {
        $res = $this->results->updateSingleResult1($_POST);
        if($res=='true') {
            echo json_encode(array("success"=>true));
        } else {
            echo json_encode(array("success"=>false));
    ...
    public function special_report() {
        if(isset($_GET['req_id'])){
            ...
            $data = array();
            $data['results'] = $this->results->GetCompletedTestRequestsById($_GET['req_id']);
            $this->pdf->load_view("mri_spcl_report",$data);
            $this->pdf->render();
            //$name="report"+$_GET['req_id']+".pdf";
    ...
    public function special_report_all() {
        if(isset($_POST['ids'])){
            ...
            for($i=0;$i<count($_POST['ids']);$i++){
                $data['results'][] = $this->results->GetCompletedTestRequestsById($_POST['ids'][$i]);
            }
