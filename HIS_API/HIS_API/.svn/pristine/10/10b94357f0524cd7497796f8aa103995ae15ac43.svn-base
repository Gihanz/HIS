...
/*			 JSONArray permissions=(JSONArray)uJson.get("userPermissions");
	...
	public String getAllActiveUserDetails(){
		...
		result = serializor.include("adminUserroles.roleId","adminUserroles.roleName","adminUserroles.adminPermissions.permissionDiscription",
									"employees.empId","employees.firstName","employees.lastName","employees.hrDepartments")
	...
	public String getUserDetailsByUserID(@PathParam("uID")  int uID){
		...
		result = serializor.include("userId","userName","hrEmployee.empId","password","adminUserroles.roleId","adminUserroles.roleName",
									"adminUserroles.adminPermissions.permissionDiscription","adminUserroles.adminPermissions.permissionId","employees.hrDepartments")
	...
	public  String updateUserDetails(JSONObject jsnUser){
		...
		try{
			...
			int usrID=jsnUser.getInt("userId");
			
			List<AdminUser> userList=new ArrayList<AdminUser>();
			
			userList = userDBDriver.getUserDetailsByUserID(usrID);
...
//		}
	...
	@Produces(MediaType.TEXT_PLAIN)
	@Consumes(MediaType.APPLICATION_JSON)
	public  String updateUserName(JSONObject jsnUser){
		...
		try{
			...
			r=userDBDriver.updateUserDetails(usrObj);
			result=String.valueOf(r);
			return result;
...
//		}
	...
	@GET
	@Path("/getUserRolePermission/{userId}/{roleId}")
	@Produces(MediaType.APPLICATION_JSON)
	public String getAllUserRole(@PathParam("userId")  int userId,@PathParam("roleId")  int roleId){
		String result="";
		List<AdminUser> usrRoleList =userDBDriver.getUserRolePermissionById(userId,roleId);

		JSONSerializer serializor=new JSONSerializer();
		result = serializor.include("userId","adminUserroles","adminUserroles.adminPermissions").exclude("*.class" ,"adminUser.hrEmployee").serialize(usrRoleList);
		return result;
	}
