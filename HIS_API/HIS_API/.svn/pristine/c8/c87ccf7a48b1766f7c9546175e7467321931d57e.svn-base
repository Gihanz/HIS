	...
	public  String userValidation(JSONObject jsnObj) throws org.codehaus.jettison.json.JSONException{
		...
		try{
			...
			if(mriUserDBDriver.validateUserLoginDetails(usr)){
				...
				result = serializor.include("userId","mriUserRoles.roleName","mriUserRoles.roleId","mriEmployee.name","userName","adminUserroles.adminPermissions.permissionDiscription").exclude("*").serialize(validUser);
