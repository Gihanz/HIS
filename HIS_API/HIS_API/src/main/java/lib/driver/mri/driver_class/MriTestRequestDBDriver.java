...
public class MriTestRequestDBDriver {
	...
	public List<MriTestRequest> GetAllTestRequestsSpecimans() {
		Session session=SessionFactoryUtil.getSessionFactory().openSession();
		Transaction tx = null;
		tx = session.beginTransaction();

		Query query = session.createQuery("from MriTestRequest t where t.mriSpecimen IS NOT NULL ");
		List<MriTestRequest> testRequests= query.list();
		tx.commit();
	...
	public Integer GetLastRequestID() {
		...
		Transaction tx = null;
		tx = session.beginTransaction();
		//Kanchana increment change 7 7 16
		Query query = session.createQuery("select MAX(r.increment) from MriTestRequest r");
		//Query query = session.createQuery("select MAX(r.testRequestId) from MriTestRequest r");
	...
	public Boolean UpdateTestRequest(MriTestRequest testRequest,int laboratoryTestId) {
		Session session=SessionFactoryUtil.getSessionFactory().openSession();
		Transaction tx=null;

		int testRequestId=testRequest.getTestRequestId();


		try {
			System.out.println("Inside UpdateTestRequest!");
			System.out.println("Test Request ID"+testRequestId);
			tx= session.beginTransaction();
			MriTestRequest updateTestRequest=(MriTestRequest) session.get(MriTestRequest.class, testRequestId);
			//updateTestRequest.setTestRequestType(testRequest.getTestRequestType());
			updateTestRequest.setTestDueDate(testRequest.getTestDueDate());
			updateTestRequest.setTestPriority(testRequest.getTestPriority());
			
			
			
			//Query query = session.createQuery("update MriTestRequest set testPriority = '"+testRequest.getTestPriority()+"' "
			      //  + "where testRequestId="+testRequestId+"");
			//query.executeUpdate();
			
			MriLaboratoryTest requestTest=(MriLaboratoryTest) session.get(MriLaboratoryTest.class, laboratoryTestId);
			updateTestRequest.setMriLaboratoryTest(requestTest);
			
			session.update(updateTestRequest);
			tx.commit();
			return true;
	...
	public Boolean UpdateTestRequestComments(MriTestRequest testRequest) {
		Session session=SessionFactoryUtil.getSessionFactory().openSession();
		Transaction tx=null;
		...
		try {
			...
			updateTestRequest.setComments(testRequest.getComments());
