...
"AND l.is_binary =1 AND (m.status =0 OR m.status =3) ORDER BY m.test_request_id";
	...
	public List GetTestRequestForRequastIDRange(int start,int end) {
		...
		String sql = "SELECT * FROM mri_test_request r, mri_laboratory_test l, mri_patient p where r.status=0 and r.flaboratory_test_id=l.laboratory_test_id and"
				+ " r.fpatient_id=p.patient_id and l.is_binary=1 and test_request_id>='"+start+"' and test_request_id<='"+end+" AND (r.status =0 OR r.status =3)'";
		SQLQuery query = session.createSQLQuery(sql);
		query.setResultTransformer(Criteria.ALIAS_TO_ENTITY_MAP);
		List results = query.list();
	...
	public List GetTestForIDRangeResult (int start,int end) {
		System.out.println("GetTestRequestForRequastIDRange Result enter");
		...
		String sql = "SELECT * FROM mri_test_request r, mri_laboratory_test l, mri_patient p where r.status=0 and r.flaboratory_test_id=l.laboratory_test_id and"
				+ " r.fpatient_id=p.patient_id and l.is_binary=1 and test_request_id>='"+start+"' and test_request_id<='"+end+"'";
		SQLQuery query = session.createSQLQuery(sql);
		query.setResultTransformer(Criteria.ALIAS_TO_ENTITY_MAP);
		List results = query.list();
