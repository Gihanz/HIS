...
public class MriSpecimenResources {
	...
	public String InsertNewSpecimen(JSONObject pJson) {
		try {
			System.out.println("InsertNewSpecimen");
			...
			//Insert User 25 06 16
			newSpecimen.setInsertedBy(data.getJSONObject(0).getInt("InsertUserID"));
			
			String requestIDs=data.getJSONObject(0).getString("requestID");
			System.out.println("InsertNewSpecimen requestID"+requestIDs);
			
			MriSpecimen addedSpecimen= specimenDBDriver.InsertNewSpecimen(newSpecimen);
			int specimentID=addedSpecimen.getSpecimenId();
			System.out.println("InsertNewSpecimen Return Speciam ID"+specimentID);
			String success="";
			if (specimentID > 0){
				...
				for(int i=0; i<requests.length;i++){
					...
					System.out.println(requestID+"Primary Key "+requestPrimaryKey);
	...
	public String getSpecimenIDsByRequestID(
			@PathParam("testRequestId1") String testRequestId1,
			@PathParam("testRequestId2")String testRequestId2) {
		
		System.out.println("Barcode Grenarate Model getSpecimenIDsByRequestID- Reqest IDs "+testRequestId1+"-"+testRequestId2);
		//String requestID=requests[i].replaceAll("\\p{Z}","");
		int requestPrimaryKey1=mriTestRequestDBDriver.GetRequestPrimaryKeyByRequestID(testRequestId1.replaceAll("\\p{Z}",""));
		int requestPrimaryKey2=mriTestRequestDBDriver.GetRequestPrimaryKeyByRequestID(testRequestId2.replaceAll("\\p{Z}",""));
		
		System.out.println(testRequestId1+"Primary Key1 "+requestPrimaryKey1+"-"+testRequestId2+"Primary Key 2"+requestPrimaryKey2 );
		
		List<MriTestRequest> requestRec = specimenDBDriver
				.retrieveTestRequestDetailByRequestIDs(requestPrimaryKey1,
						requestPrimaryKey2);
		JSONSerializer serializer = new JSONSerializer();
		
		

		 for(MriTestRequest model : requestRec) {
	            System.out.println(model.getRequestId()+" TEST REQUEST");
	        }
	...
	public String getSpecimenIDsByRequestIDs(
		...
		System.out.println("Get ID From MriTestRequest Table  getSpecimenIDsByRequestID- Reqest IDs "+testRequestId1+"-"+testRequestId2);
	...
	public String GetRequestByRequestID(@PathParam("requestID") int requestID) {
		String result = "";
		try {
			System.out.println("MRI Speciman  GetRequestByRequestID- Reqest IDs "+requestID);
	...
	// *2 time Get ID From MriTestRequest Table getSpecimenIDsByRequestID
	@GET
	...
	public String getSpecimenIDsByRequestID(
		...
		try {
			List<MriTestRequest> requestRec = specimenDBDriver
			 ...
			 for(MriTestRequest model : requestRec) {
		            System.out.println("Request IDDS"+model.getRequestId());
		        }
