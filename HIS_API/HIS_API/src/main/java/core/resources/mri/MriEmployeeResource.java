	...
	public String GetAllEmployees() {
		...
		try {
			List<MriEmployee> employees= mriEmployeeDBDriver.GetAllEmployees();
			 ...
			 for(MriEmployee model : employees) {
		            System.out.println(model.getEmployeeId().toString()+""+model.getMriUsers());
		           // MriUser user= model.getMriUsers();
		        }

			JSONSerializer serializer = new JSONSerializer();

			return serializer.include("mriUsers.userName").exclude("*.class").serialize(employees);
