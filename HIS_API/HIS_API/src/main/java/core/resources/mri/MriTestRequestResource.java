...
public class MriTestRequestResource {
	...
	@GET
	@Path("/GetAllTestRequestWithSpecimen")
	@Produces(MediaType.APPLICATION_JSON)
	public String GetAllTestRequestWithSpecimen() {
		String result="";
		try {
			List<MriTestRequest> testRequests= mriTestRequestDBDriver.GetAllTestRequestsSpecimans();

			JSONSerializer serializer = new JSONSerializer();

			return serializer.exclude("mriLaboratoryTest").exclude("mriPatient").include("MriSpecimen.*").exclude("*.class").serialize(testRequests);
	...
	public String InsertNewInternalRequests(JSONObject pJson)
	{
		try {
				...
				for (int curr = 0; curr < data.length(); curr++){
						...
						//Inserted User Kanchana 2 25 06 16
						testRequest.setRequestedBy(data.getJSONObject(curr).getInt("InsertUserID"));
...
//						}
	...
	public String GetInternalRequestsBySearch(@PathParam("data1")  String searchParam,@PathParam("data2")  String searchID) {
		String result="";
		try {
			System.out.println("searchParam : "+searchParam +" searchID: "+searchID);
	...
	public String UpdatePatient(JSONObject pJson)
	{
		try {
				...
				JSONArray data = pJson.getJSONArray("testrequest");

				MriTestRequest testRequest= new MriTestRequest();
				testRequest.setTestRequestId(Integer.parseInt(data.getJSONObject(0).getString("testRequestId")));	
				
				
				int laboratoryTestId=data.getJSONObject(0).getInt("mriLaboratoryTest");
					System.out.println("Before Update"+laboratoryTestId);
				//testRequest.setTestRequestType(testRequestType);
			
				testRequest.setTestDueDate(dateformat.parse(data.getJSONObject(0).getString("testDueDate")));
				testRequest.setTestPriority(data.getJSONObject(0).getString("testPriority"));
					Boolean testRequestUpdated=mriTestRequestDBDriver.UpdateTestRequest(testRequest,laboratoryTestId);
	...
	//Test Verify and Add Results 2016/06/20
	@POST
	@Path("/UpdateTestRequestComments")
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	public String UpdateTestRequestComment(JSONObject pJson)
	{
		try {
				...
				MriTestRequest testRequest= new MriTestRequest();
				testRequest.setTestRequestId(Integer.parseInt(data.getJSONObject(0).getString("testRequestId")));	
				testRequest.setTestPriority(data.getJSONObject(0).getString("testRequestComment"));
				
				Boolean testRequestUpdated=mriTestRequestDBDriver.UpdateTestRequestComments(testRequest);
